...
public class CurrentGradeLoaderRemoveNG implements Loader {
	...
	public double[][] readAllStudents() throws FileNotFoundException {
		...
		ArrayList<ArrayList<Double>> allClasses;
		try {
			...
			// DEAL WITH NG's
			allClasses = new ArrayList<ArrayList<Double>>(allStudents[0].length);
			//add all the classes
			...
			for (int col = 0; col < allStudents[0].length; col++) {
				for (int row = 0; row < allStudents.length; row++) {
					if (allStudents[row][col] != -1) {
						allClasses.get(col).add(allStudents[row][col]);
					}
				}
			}
